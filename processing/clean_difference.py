...
class CleanDifference(Module):
    def __init__(self):
        super().__init__()
        self.foreground_in = Input(data_type=CVImage, config_keys=['cam_ids'])
        self.diff_out = Output(data_type=CVImage, config_keys=['cam_ids'])
    ...
    def process_foreground_in(self, fg):
        ...
        self.diff_out.data_ready(CVImage(opened, fg.id, fg.camera_info))
